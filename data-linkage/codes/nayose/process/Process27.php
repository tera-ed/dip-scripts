...
/**
 * OBIC billing-LBC matching file: nayose integration process
 *
...
class Process27{
	...
	const WK_T_TABLE = 'wk_t_nayose_sei_result';
	const WK_BATCH_SYNC_NO_KEY_NAME = 'wk_t_nayose_sei_result_seq_no';

	const KEY_1 = 'billing_code';
	const KEY_2 = 'office_id';

	const CSV_UNIQUE = 0;
	const CSV_MULTIPLE = 1;

	const SHELL = 'load_wk_t_nayose_sei_result.sh';

	/**
	 * Process27 Class constructor
	 */
				...
				foreach ($importFiles as $importFile){
					...
					foreach ($importFile as &$file){
						if($commited === true && shellExecAddSyncNo($this->logger, self::SHELL, $file, self::WK_T_TABLE, $batchSyncNo) === 0){
							...
							throw new Exception("Process27 Failed to insert with " . self::SHELL . " to " . self::WK_T_TABLE);
						}
					}
				}
	...
	private function reqData($batchSyncNo){
		try { // execute nayose process
			// request data
			$result = $this->db->getData("billing_code, office_id, result_flg, detail_lvl, detail_content",
			self::WK_T_TABLE, " delete_flag IS NULL AND billing_code NOT LIKE '%00' AND batch_sync_seq_no = '".$batchSyncNo."'", null);
		} catch (PDOException $e1){
			throw $e1;
		} catch (Exception $e2){
			throw $e2;
