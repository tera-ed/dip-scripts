...
/**
 * OBIC contract-LBC matching file: nayose integration process
 *
...
class Process26{
	...
	const KEY_1 = 'contract_code';
	const KEY_2 = 'office_id';

	const CSV_UNIQUE = 0;
	const CSV_MULTIPLE = 1;

	const SHELL = 'load_wk_t_nayose_kei_result.sh';

	/**
	 * Process26 Class constructor
	 */
				...
				foreach ($importFiles as $importFile){
					...
					foreach ($importFile as &$file){
						if($commited === true && shellExecAddSyncNo($this->logger, self::SHELL, $file, self::WK_T_TABLE, $batchSyncNo) === 0){
							...
							throw new Exception("Process26 Failed to insert with " . self::SHELL . " to " . self::WK_T_TABLE);
						}
					}
				}
	...
	private function reqData($batchSyncNo){
		try { // execute nayose process
			// request data
			$result = $this->db->getData("contract_code, office_id, result_flg, detail_lvl, detail_content",
			self::WK_T_TABLE, " delete_flag IS NULL AND contract_code LIKE '%00' AND batch_sync_seq_no = '".$batchSyncNo."'", null);
		} catch (PDOException $e1){
			throw $e1;
		} catch (Exception $e2){
			throw $e2;
