...
function getMultipleCsv($filename, $path){
  ...
  try{
    ...
    if(empty($fileList))
      throw new Exception("File not found.".$path."*".$filename."*.csv", 602);
  } catch(Exception $e){
    throw $e;
  }
  return $fileList;
...
function getImportPath($isAfter = false){
  global $root,$fldName, $IMPORT_PATH;
  $mode = $isAfter? 'after' : 'before';
  return $root.$IMPORT_PATH[$mode]."/".$fldName."/";
}
...
function getExportPath($isAfter = false){
  global $root,$fldName, $EXPORT_PATH;
  $mode = $isAfter? 'after' : 'before';
  return $root.$EXPORT_PATH[$mode]."/".$fldName."/";
}
 ...
 * @return string export path
 */
function shellExec($logger, $shellPass, $csvFilePass) {
  ...
  try{
    if($logger == null || $logger ==''){
      ...
      $cmd = 'bash '.$root.$IMPORT_PATH['shellDir'].$shellPass.' '.$csvFilePass.' '.$logFilename.' 2>&1';
